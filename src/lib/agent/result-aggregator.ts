...
import type { CanonicalContent, ModelReview, ReviewResult, RuleResult } from "@/lib/schemas/review";
import { findEvidenceSegment } from "@/lib/validation/evidence";

function humanReviewRequired(ruleId: RuleResult["ruleId"], status: RuleResult["status"]): boolean {
  return (ruleId === "A-06" && status === "RISK")
    || (ruleId === "A-09" && status === "UNCERTAIN")
    || (ruleId === "A-10" && status === "UNCERTAIN");
}
...
export function aggregateReview(
      ...
      evidenceRefs: item.evidence.flatMap((quote) => {
        ...
        return segment ? [{
          ...
          locationConfidence: segment.bbox
            ? (segment.confidence ?? 0) >= 0.9 ? "EXACT" as const : "APPROXIMATE" as const
            : null,
        }] : [];
      }),
      reason: item.reason,
  ...
  const uncertainResults = ruleResults.filter((result) => result.status === "UNCERTAIN");
  const overallStatus = hasRisk ? "RISK" : uncertainResults.length > 0 ? "UNCERTAIN" : "PASS";
...
export function safeFallback(trace: ReviewResult["trace"], message: string): ReviewResult {
  const modelReview: ModelReview = {
    ruleResults: ["A-01", "A-02", "A-03", "A-04", "A-05", "A-06", "A-07", "A-08", "A-09", "A-10"].map((ruleId) => ({
  ...
  const result = aggregateReview(modelReview, {
    inputType: "text",
    rawUserText: null,
